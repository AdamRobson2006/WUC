<?php
require "../includes/Portal-Mockup-HEADER.html";
?>

<h1 >Timetable</h1>

<hr id="gap">

<!--STUDENT NAME -->
<!--I think something we can do here is use a calendar API, this is primarily because it can speed up the process-->
<table id="timetable">
		<tr>
            <td></td>
			<td class="Date">13/04/2026</td>
			<td class="Date">14/04/2026</td>
			<td class="Date">15/04/2026</td>
			<td class="Date">16/04/2026</td>
			<td class="Date">17/04/2026</td>
			<td class="Date">18/04/2026</td>
			<td class="Date">19/04/2026</td>
		</tr>
        <tr>
			<td class="Time">Time</td>
			<td class="weekDay">Monday</td>
			<td class="weekDay">Tuesday</td>
			<td class="weekDay">Wednesday</td>
			<td class="weekDay">Thursday</td>
			<td class="weekDay">Friday</td>
			<td class="weekEnd">Saturday</td>
			<td class="weekEnd">Sunday</td>
		</tr>
		<tr>
			<td class="Time">08:00 - 09:00</td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slotEnd"></td>
			<td class="slotEnd"></td>
		</tr>
		<tr>
            <td class="Time">09:00 - 10:00</td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slotEnd"></td>
			<td class="slotEnd"></td>
		</tr>
		<tr>
			<td class="Time">10:00 - 11:00</td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slotEnd"></td>
			<td class="slotEnd"></td>
		</tr>
		<tr>
			<td class="Time">11:00 - 12:00</td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slotEnd"></td>
			<td class="slotEnd"></td>
		</tr>
		<tr>
			<td class="Time">12:00 - 13:00</td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slotEnd"></td>
			<td class="slotEnd"></td>
		</tr>
		<tr>
			<td class="Time">13:00 - 14:00</td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slotEnd"></td>
			<td class="slotEnd"></td>
		</tr>
        <tr>
			<td class="Time">13:00 - 14:00</td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot" ></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slotEnd"></td>
			<td class="slotEnd"></td>
		</tr>
        <tr>
            <td class="Time">14:00 - 15:00</td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slotEnd"></td>
            <td class="slotEnd"></td>
        </tr>
        <tr>
            <td class="Time">15:00 - 16:00</td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
            <td class="slot"></td>
			<td class="slotEnd"></td>
			<td class="slotEnd"></td>
        </tr>
        <tr>
            <td class="Time">16:00 - 17:00</td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slot"></td>
			<td class="slotEnd"></td>
			<td class="slotEnd"></td>
        </tr>
</table>

<?php
require "../Includes/Portal-Mockup-FOOTER.html";
?>